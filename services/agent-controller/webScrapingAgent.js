...
class WebScrapingService {
    ...
    try {
      ...
      switch (options.type) {
        case 'scrape-political-content':
          return await this.scrapePoliticalContent(options.keywords, options.sources);
        
        case 'scrape-business-content':
          return await this.scrapeBusinessContent(options.keywords, options.sources);
   ...
   */
  async scrapePoliticalContent(keywords = [], sources = []) {
    try {
   ...
   */
  async scrapeBusinessContent(keywords = [], sources = []) {
    try {
      console.log(`🔍 Scraping business content with keywords: ${keywords ? keywords.join(', ') : 'none'}`);
      console.log(`📚 Using sources: ${sources && sources.length > 0 ? sources.join(', ') : 'all business'}`);

      // Handle case where keywords might be undefined
      const safeKeywords = keywords && Array.isArray(keywords) ? keywords : [];

      // Use all business sources if none specified
      const sourcesToUse = sources && sources.length > 0 ? sources : ['youtube', 'twitter', 'tiktok', 'instagram'];

      // Validate sources
      const validSources = sourcesToUse.filter(source => this.supportedPlatforms[source]);
      if (validSources.length === 0) {
        throw new Error('No valid business sources specified');
      }
